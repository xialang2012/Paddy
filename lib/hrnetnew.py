...
def conv(x, outsize, kernel_size, strides_=1, padding_='same', activation=None):
    return Conv2D(outsize, kernel_size, strides=strides_, padding=padding_, kernel_initializer='he_normal', use_bias=False, activation=activation)(x)
...
def HRNetNew(input_shape, nClass):
    channels_2 = [32, 64]
    ...
    up2 = Conv2D(nClass, (1, 1), strides=(1, 1),padding='same',use_bias=False)(up2)
    
    up2 = Activation('sigmoid', name='Classification')(up2)
    model = Model(inputs, up2)
    return model