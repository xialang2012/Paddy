...
class IOProcess():
    ...
    def ReadH5(self, fileName, datsetName):
        in_ds = h5py.File(fileName, 'r')
        # /HDFEOS/GRIDS/VNP14A1_Grid/Data Fields/MaxFRP
        array = in_ds[datsetName][:]
        #MaxFRP = in_ds['/HDFEOS/GRIDS/VNP14A1_Grid/Data Fields/MaxFRP'][:]
        in_ds.close()
        return array

    def ReadHDF4(self, fileName, datsetName):
        hdf_obj = SD.SD(fileName, SD.SDC.READ)
        array = hdf_obj.select(datsetName)[:,:]
        #print(Map[:,:])
        hdf_obj.end()
        return array

    def ReadNC(self, fileName, groupName, datasetName, x = -1, y = -1, layer = -1, noAutoScale = True):
        nc_obj = Dataset(fileName)
        if noAutoScale: nc_obj.set_auto_scale(False)
        data = 0
        if groupName == '':
            data = np.squeeze(np.array(nc_obj.variables[datasetName][:]))
        else:
            data = np.squeeze(np.array(nc_obj.groups[groupName][datasetName]))
        nc_obj.close()

        if layer != -1:
            return np.squeeze(data[layer,:,:])
        if x == -1 and y == -1:
            return data
        else:
            return data[:, x, y]
